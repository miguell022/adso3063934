...
export default function EditGenerationScreen({ navigation, route }) {
  const { token } = useAuth();

  // 1️⃣ Recibimos la generación que viene desde GenerationsScreen
  const { generation } = route.params;

  // 2️⃣ Inicializamos los campos con los datos existentes
  const [name, setName] = useState(generation.name);
  const [years, setYears] = useState(generation.years);
  const [consoles, setConsoles] = useState(generation.main_consoles);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // 3️⃣ Hacemos PUT con el id de la generación a editar
      const response = await fetch(
        `${API_URL}/console-generations/${generation.id}`,
        {
          method: "PUT",
          headers: {
      ...
      if (!response.ok) {
        Alert.alert("Error", data.error || "No se pudo actualizar la generación.");
        return;
      }

      Alert.alert("Generación actualizada", `"${name}" fue actualizada exitosamente.`, [
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Text style={styles.title}>Editar generación</Text>
